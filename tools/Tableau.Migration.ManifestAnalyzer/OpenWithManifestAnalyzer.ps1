...
try {
    ...
    $programPath = Join-Path $scriptDir "Tableau.Migration.ManifestAnalyzer.exe"

    # Define the registry key path for .json files
    $regPath = "HKLM:\Software\Classes\SystemFileAssociations\.json\shell\OpenWithManifestAnalyzer"

    # Create the registry key for the context menu item and set the default value
    New-Item -Path $regPath -Force -Value "Open with Manifest Analyzer" -ErrorAction Stop | Out-Null
    ...
    $iconPath = Join-Path $scriptDir "ManifestAnalyzerIcon.ico"
