...
# Directory for C# API documentation. Includes an index file and a directory for DocFX generated files.
$csharp_apiref_dir = Join-Path $main_docs_dir "api-csharp";
...
.PARAMETER Exclude
	The file/directory patterns to exclude from deletion.
#>
	...
	if ($SkipPreClean) {
		...
		Get-ChildItem -Path ($path + "/") * -Exclude $Exclude -Directory -Recurse | ForEach-Object { Remove-Item $_ -Recurse };
		Get-ChildItem -Path ($path + "/") * -Exclude $Exclude -File -Recurse | ForEach-Object { Remove-Item $_ };
...
#>
	...
	Run-Command ("python -m pip install -q hatch click==8.2");
...
#>
	...
	Run-Command ("Clear-Directory -Path $csharp_apiref_dir -Exclude '.gitignore','index.md'");
