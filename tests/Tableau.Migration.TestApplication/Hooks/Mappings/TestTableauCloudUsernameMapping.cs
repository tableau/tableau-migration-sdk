...
namespace Tableau.Migration.TestApplication.Hooks.Mappings
{
    /// <summary>
    ...
    public class TestTableauCloudUsernameMapping : ContentMappingBase<IUser>, ITableauCloudUsernameMapping
    {
        ...
        public override Task<ContentMappingContext<IUser>?> MapAsync(ContentMappingContext<IUser> ctx, CancellationToken cancel)
        {
            ...
            if (string.Equals(_baseEmail.User, userName, StringComparison.OrdinalIgnoreCase))
            {                
                return ctx.MapTo(domain.Append(_baseEmail.ToString())).ToTask();
            }

            var testEmail = $"{_baseEmail.User}+{userName.Replace(" ", "")}@{_baseEmail.Host}";
