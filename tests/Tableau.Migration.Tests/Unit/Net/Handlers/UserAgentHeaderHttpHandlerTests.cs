...
{
    public class UserAgentHeaderHttpHandlerTests
    {
        private readonly Mock<IUserAgentProvider> _mockedUserAgentProvider;

        public UserAgentHeaderHttpHandlerTests()
        {
        ...
        public void CallSendAsync_AttachUserAgentHeader()
        {
            ...
            var handler = new UserAgentHeaderHttpHandler(_mockedUserAgentProvider.Object)
            {
                InnerHandler = Mock.Of<HttpMessageHandler>()
            };
            var methodInfo = typeof(UserAgentHeaderHttpHandler).GetMethod("SendAsync", BindingFlags.NonPublic | BindingFlags.Instance)!;
