...
{
    public class DestinationContentReferenceFinderFactoryTests
    {
        public class ForContentType : AutoFixtureTestBase
        {
            ...
            public void ReturnsManifestDestinationFinder()
            {
                ...
                var fac = Create<DestinationContentReferenceFinderFactory>();

                var finder = fac.ForContentType<TestContentType>();

                mockPipeline.Verify(x => x.CreateDestinationContentReferenceFinder<TestContentType>(), Times.Once);
