...
{
    public class SourceContentReferenceFinderFactoryTests
    {
        public class ForContentType : AutoFixtureTestBase
        {
            ...
            public void ReturnsManifestSourceFinder()
            {
                ...
                var fac = Create<SourceContentReferenceFinderFactory>();

                var finder = fac.ForContentType<TestContentType>();

                mockPipeline.Verify(x => x.CreateSourceContentReferenceFinder<TestContentType>(), Times.Once);

                Assert.IsAssignableFrom<ISourceContentReferenceFinder<TestContentType>>(finder);
