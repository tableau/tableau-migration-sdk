...
{
    public abstract class NameSearchApiClientTestBase<TApiClient, TContent, TResponse> : 
        FilteredPagedListApiClientTestBase<TApiClient, TContent, TResponse>
        where TApiClient : INameSearchApiClient<TContent>, IContentApiClient
        where TResponse : TableauServerResponse, new()
    {

        [Fact]
        public async Task SearchNameAsync()
        {
            // Arrange
            ...
            // Act
            var results = await ApiClient.SearchByNameAsync("testName", 123, Cancel);
            ...
            request.AssertQuery("filter", GetExpectedFilterExpression([new Filter("name", ApiClient.NameFilterOperator, "testName")]));
        }
    }
}
