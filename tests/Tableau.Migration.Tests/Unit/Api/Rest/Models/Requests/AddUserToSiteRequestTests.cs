...
{
    public sealed class AddUserToSiteRequestTests
    {
        public sealed class Ctor : AutoFixtureTestBase
        {
            [Fact]
            public void SetsIdpConfigurationName()
            {
                var auth = UserAuthenticationType.ForConfigurationId(Guid.NewGuid());
                var r = new AddUserToSiteRequest("name", "siteRole", auth);

                Assert.NotNull(r.User);
                Assert.Equal("name", r.User.Name);
