    ...
    public class GetSubscriptionsResponseTests
    {
        public class Serialization : SerializationTestBase
        {
            ...
            public void DeSerializes()
            {
                ...
                var xml = BuildInputXml(expectedSubscriptions);
                ...
                static void AssertSchedule(GetSubscriptionsResponse.SubscriptionType.ScheduleType? expected, GetSubscriptionsResponse.SubscriptionType.ScheduleType? actual)
                {
                    ...
                    Assert.Equal(expected?.NextRunAt, actual?.NextRunAt);

                    AssertFrequencyDetails(expected?.FrequencyDetails, actual?.FrequencyDetails);
                }

                static void AssertFrequencyDetails(
                    GetSubscriptionsResponse.SubscriptionType.ScheduleType.FrequencyDetailsType? expected,
                    GetSubscriptionsResponse.SubscriptionType.ScheduleType.FrequencyDetailsType? actual)
                {
                    ...
                    Assert.Equal(expected.Intervals.Length, actual.Intervals.Length);
                }

                static void AssertUser(
                    GetSubscriptionsResponse.SubscriptionType.UserType? expected,
                    GetSubscriptionsResponse.SubscriptionType.UserType? actual)
                {
                    Assert.NotNull(actual);
            ...
            private static string BuildInputXml(List<GetSubscriptionsResponse.SubscriptionType> expectedSubscriptions)
            {
                var builder = new StringBuilder();

                AppendTsResponseStartElement(builder);
                AppendPaginationElement(builder, 1, 100, expectedSubscriptions.Count);
        ...
        <subscription id=""{item.Id}"" subject=""{item.Subject}"" attachImage=""{item.AttachImage.ToString().ToLower()}"" attachPdf=""{item.AttachPdf.ToString().ToLower()}"" suspended=""{item.Suspended.ToString().ToLower()}"">");
            ...
            <content id=""{item.Content?.Id}"" type=""{item.Content?.Type}"" sendIfViewEmpty=""{item.Content?.SendIfViewEmpty.ToString().ToLower()}""/>");
                    ...
                    builder.Append($@"
            <schedule frequency=""{item.Schedule?.Frequency}"" nextRunAt=""{item.Schedule?.NextRunAt}"">");
                    var frequencyDetails = item.Schedule?.FrequencyDetails;
                    if (frequencyDetails is not null)
                    {
                        builder.Append($@"
                <frequencyDetails start=""{frequencyDetails.Start}"" end=""{frequencyDetails.End}"">");
                        AppendIntervals(builder, frequencyDetails?.Intervals);
                        builder.Append($@"                 
                </frequencyDetails>");
                    }
                    builder.Append($@"
            </schedule>");
                    builder.Append($@"            <user id=""{item.User?.Id}"" name=""{item.User?.Name}""/>");
                    builder.Append($@"        </subscription>");
                }

                builder.AppendLine(@"    </subscriptions>");
                AppendTsResponseEndElement(builder);
                return builder.ToString();

                static void AppendIntervals(
                    StringBuilder xmlBuilder,
                    GetSubscriptionsResponse.SubscriptionType.ScheduleType.FrequencyDetailsType.IntervalType[]? intervals)
                {
                    ...
                    {
                        xmlBuilder.Append($@"
                        <interval hours=""{interval.Hours}"" minutes=""{interval.Minutes}"" weekDay=""{interval.WeekDay}"" monthDay=""{interval.MonthDay}""/>
                        ");
                    }
