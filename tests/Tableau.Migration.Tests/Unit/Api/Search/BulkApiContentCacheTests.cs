...
using Tableau.Migration.Tests.Unit.Engine.Endpoints.Caching;
using Xunit;

namespace Tableau.Migration.Tests.Unit.Api.Search
{
    public class BulkApiContentCacheTests
    {
        public abstract class ApiContentCacheTest : ApiContentReferenceCacheBaseTest<BulkApiContentCache<TestContentType>, TestContentType>
        {
            protected readonly ConcurrentDictionary<ContentReferenceStub, TestContentType> InnerCache = new();

            protected readonly Mock<BulkApiContentCache<TestContentType>> MockCache;

            protected readonly Mock<ILogger<BulkApiContentCache<TestContentType>>> MockLogger;

            protected Mock<IContentReferenceCache> MockReferenceCache => MockCache.As<IContentReferenceCache>();

            public ApiContentCacheTest()
            {
                MockLogger = Create<Mock<ILogger<BulkApiContentCache<TestContentType>>>>();
                MockCache = Mock.Get(Cache);
            }

            protected override BulkApiContentCache<TestContentType> CreateCache()
                => new Mock<BulkApiContentCache<TestContentType>>(
        ...
        {
            [Fact]
            public async Task ReturnsCachedContentWhenFoundAsync()
            {
            ...
            [Fact]
            public async Task ReturnsNullWhenNotFoundAsync()
            {
